...
3）模块编译：
		...
		① .js文件编译：
				fs模块读取文件后编译执行，正常js模块会被包装成function(exports, require, module, --filename, --dirname) {\n 代码块 \n}，通过vm原生模块执行，this指向该模块本身；
		② .node文件编译：
				这是C/C++编写的扩展文件，通过process.dlopen()方法加载最后编译生成的结果，针对不同平台libuv进行封装，实际上，该部分并不需要编译，因为是C/C++文件编译后生成的；
		③ .json文件编译：
				fs模块读取文件后用JSON.parse()解析返回结果，该部分最简单；
		④ 其他扩展名文件：
				均当成.js文件进行解析；
